...
async def plus_or_minus(message, cleared_text):
    if karmic_triggers.PLUS == cleared_text:
        ...
        await has_plus_karma(message)
    elif karmic_triggers.MINUS == cleared_text:
        await has_minus_karma(message)
    elif karmic_triggers.MINUS == cleared_text[0]:
        await has_minus_karma(message, cleared_text[1:])
    ...
    if interger != 0:
        try:
            ...
            if result > 0:
                ...
                await message.reply(f'У Вас слишком мало кармы для этого')
        except:
            pass
    else:
        result = db.get_karma_user(message.chat.id, message.from_user.id)
